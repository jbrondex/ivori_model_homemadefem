            ...
            for IP in range(el.numberofIP):
                ...
                for p in range(el.numberofnodes): ###loop on matrice lines (correspond to test functions )
                    for q in range(el.numberofnodes): ###loop on matrices columns (correspond to trial functions )
                        ...
                        Kloc[Solver.DOFs*p,Solver.DOFs*q] += Deff * dbasis[q] * dbasis[p] *  IP_weight * el_size ##K_{PP}
                        Kloc[Solver.DOFs*p,Solver.DOFs*q+2] += basis[q] * basis[p] *  IP_weight * el_size ##K_{PC}                        
                        Kloc[Solver.DOFs*p+1,Solver.DOFs*q+1] += Keff * dbasis[q] * dbasis[p] *  IP_weight * el_size  ##K_{TT}
                        Kloc[Solver.DOFs*p+1,Solver.DOFs*q+2] += - Lm * basis[q] * basis[p] *  IP_weight * el_size  ##K_{TC}
...
####~~~~~ Update the solution fields for next timestep ~~~~~#### 
    for i in range(Solver.parent.snowpack.mesh.numberofNodes): 
        ...
        c.value[i] = Solver.solution[Solver.DOFs*i+2]
